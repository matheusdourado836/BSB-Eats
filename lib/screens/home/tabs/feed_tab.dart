...
class FeedTab extends StatefulWidget {
  ...
  const FeedTab({super.key, this.eventBus});
...
class _FeedTabState extends State<FeedTab> {
  ...
  void initState() {
    ...
    widget.eventBus?.on().listen((event) {
      ...
      if(event == 'PostDeleted' || event == 'Refresh') {
        _onRefresh();
      }
    });
    super.initState();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
            ...
            builder: (context, value, _) {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final isFollowing = _userController.currentUser?.following?.any((follower) => follower.id == user.id);
                  return ListTile(
                    minLeadingWidth: 0,
                    ...
                    isThreeLine: true,
                    onTap: () {
                      if(user.id == _userController.currentUser!.id) {
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () => showNewPostOpcoesBottomSheet(context).then((res) {
          if(res is List<String?> && res.isNotEmpty) {
            if(res.length > 10) {
              showCustomTopSnackBar(text: 'Você só pode adicionar 10 imagens');
              return;
            }
            List<String> files = res.nonNulls.toList();
            Navigator.pushNamed(context, '/upload', arguments: {"files": files, "eventBus": widget.eventBus});
          }
        }),
